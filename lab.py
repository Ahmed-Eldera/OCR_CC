...
def load_img(image_number):
    ...
    image_paths = [
        r"C:\Users\hotoe\Desktop\01 - Straightforward.jpg",
        r"C:\Users\hotoe\Desktop\02 - You can do it.jpg",
        r"C:\Users\hotoe\Desktop\03 - Should be okay.jpg",
        r"C:\Users\hotoe\Desktop\04 - Still ok, I hope.jpg",
        r"C:\Users\hotoe\Desktop\05 - Looks cool, hope it runs cool too.jpg",
        r"C:\Users\hotoe\Downloads\06 - Hatetlewe7 hatlewe7.jpg",
        r"C:\Users\hotoe\Downloads\07 - Hatet3eweg hat3eweg.jpg",
        r"C:\Users\hotoe\Downloads\08 - Ew3a soba3ak ya3am.jpg",
        r"C:\Users\hotoe\Downloads\11 - Ya setty ew3i.jpg",
        r"C:\Users\hotoe\Downloads\16 - Sheel el kart yastaaaa.jpg",
        r"C:\Users\hotoe\Downloads\15 - Compresso Espresso.jpg"
...
def filter_and_convert_lines(lines):
    """
    Filters lines from Hough Transform to keep only those with abs(x1 - x2) > 30.
    Converts the filtered lines to a NumPy array with shape (N, 1, 4).

    Parameters:
        lines (numpy.ndarray): Array of detected lines from Hough Transform.
                               Shape should be (N, 1, 4), where each line is [x1, y1, x2, y2].

    Returns:
        numpy.ndarray: Filtered and reshaped array of lines with shape (M, 1, 4).
    """
    if lines is None or len(lines) == 0:
        # Return an empty array with shape (0, 1, 4) if no lines are provided
        return np.empty((0, 1, 4), dtype=np.int32)

    # Reshape lines to a 2D array if it's not already
    if lines.ndim == 3 and lines.shape[1] == 1:
        lines = lines[:, 0]  # Remove the middle dimension

    # Filter lines where abs(x1 - x2) > 30
    filtered_lines = [line for line in lines if abs(line[0] - line[2]) > 30]

    # Convert the list of filtered lines to a NumPy array with shape (M, 1, 4)
    filtered_lines_array = np.array(filtered_lines, dtype=np.int32)

    # Reshape to (M, 1, 4)
    if filtered_lines_array.size > 0:
        filtered_lines_array = filtered_lines_array.reshape(-1, 1, 4)
    else:
        # Ensure the shape is (0, 1, 4) if no lines are left after filtering
        filtered_lines_array = np.empty((0, 1, 4), dtype=np.int32)
...
def find_perpendicular_line(longest_line, lines, angle_threshold=30):
    ...
    return None
def find_per_line(longest_line, lines, angle_threshold=30):
    """ Check if there is a line perpendicular to the longest line. """
    if longest_line is None:
        return None
...
def rotate_image(image, angle,clr=0):
    """ Rotate the image around its center by the given angle. """
    (h, w) = image.shape[:2]
    center = (w // 2, h // 2)
    M = cv2.getRotationMatrix2D(center, -angle, 1.0)
    rotated_image = cv2.warpAffine(image, M, (w, h), flags=cv2.INTER_CUBIC, borderMode=cv2.BORDER_CONSTANT, borderValue=clr)
...
def process_image_for_horizontal_longest_line(image,OG,clr):
    ...
    for i in range(len(lines)):
        ...
        if perpendicular_line is not None:
            ...
            OG = rotate_image(OG, -rotation_angle,clr)
